...
const all_content =
  document.querySelectorAll(".content"); /*All content of a tab*/
...
function fetchImages() {
  fetch("data.json") /*read a json file*/
    ...
    .then((data) => {
      let topImages = data.filter(
        (image) => image.part === "top"
      ); /*filter in json file for a top artikel*/
...
function addToCart(image) {
  ...
  if (image.part === "top") {
    choosedTop = {
      /*declare on line 258*/ url: image.url,
        ...
        if (element["part"] == kitToChoose && element["color"] == color) {
          let elemDiv =
            document.createElement(
              "div"
            ); /*create a div-container(elemDiv) of the image*/
          ...
          elemImg.addEventListener("click", (e) => {
            removeStyleAllChild(
              theElemList
            ); /*remove a style on all element into the section on parameter*/
            e.target.style =
              "border:2px solid green; border-radius:15px"; /*add a style on a clicked element*/
            if (kitToChoose == "top") {
...
// warenkorb schließen --> klick auf irgendwelche section
...
function removeCartItem(ProduktCart, index) {
  ...
  if (parseInt(korbZahl.innerHTML) == 0) {
    //schließen der Warenkorb wenn es leer ist
    warenkorbDiv.style.left = "100%";
  }
}
